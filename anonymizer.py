...
RULES = [
    ...
    },
    {
        ...
        "patterns": [
            ...
            r'\bг\.?\s+[А-ЯЁ][а-яё]+\s*,\s*(?:ул|пр|наб|пер|бульв|ш|пл)\.?\s+[А-ЯЁа-яё]' + _ADDR_CHUNK + r'(?:\s*,\s*(?:д|дом|стр|корп|к|кв|офис|пом|эт)\.?\s*' + _ADDR_CHUNK + r')+',
        ]
    },
...
def _ner_view(text):
    ...
    Длина текста не меняется — индексы совпадают с исходным."""
    return _NER_LINE_MARKER.sub(lambda m: m.group(1) + ' ', text)
