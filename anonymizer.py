...
NER_ROLE_STOPWORDS = {
    ...
    # ─── Юридический шаблон договора (добавлено 07.08.2026) ────────────────
    # «Общество» в договорах и уставах — не название компании, а способ
    # сослаться на сторону («Общество вправе…», «Уставный капитал Общества»).
    # NER принимает его за ORG, и текст устава приходит в LLM без подлежащего.
    # ⚠️ Именно на этом слове проверено, что стоп-лист НЕ снимает маскировку
    # с настоящего названия: «Общество с ограниченной ответственностью
    # «Ромашка»» по-прежнему теряет «Ромашка» — стоп-лист работает только по
    # ОДИНОЧНОМУ слову, а имя компании приходит отдельным спаном.
    "общество",
}
...
class SurnameDictLayer:
    ...
    def slovarnoe_lichnoe(self, word):
        """У слова есть СЛОВАРНЫЙ личный разбор (`Surn`/`Name`/`Patr`).

        ⚠️ Ключевое слово — словарный (`p.is_known`). У pymorphy3 есть
        угадыватель, и незнакомое слово он охотно объявляет фамилией: замер
        03.08.2026 показал, что «мсфо», «оквэд», «вебинар» так и получали
        разбор `Surn`. У настоящих фамилий и имён личные разборы словарные,
        у аббревиатур и заголовочных слов словарных личных разборов ноль.

        Используется как СТРАХОВКА: слово, про которое словарь знает, что оно
        имя, не может быть отменено стоп-листом. Границы признака замерены
        07.08.2026 и названы: на оси «фамилия против заголовка» он безошибочен
        (34 слова), но названия компаний от заголовков НЕ отличает — «АЭРОФЛОТ»
        и «РЕЗЮМЕ» для словаря одинаковы. Поэтому на нём нельзя строить правило,
        СНИМАЮЩЕЕ маскировку с капса (см. отвергнутую гипотезу в `_apply_ner`).
        """
        return any(p.is_known and (self.LICHNOE & set(p.tag.grammemes))
                   for p in self._morph.parse(word))

    def padezhnaya_forma_stop_slova(self, word, stop_slova):
        """Слово — падежная форма слова из `stop_slova` («Смету» → «смета»).

        Зачем. Стоп-лист сверялся точной формой, поэтому «Смету», «Отчёту»,
        «Планёрку» проходили мимо него и уезжали в маску, хотя именительный
        падеж в списке есть.

        ⛔ Множественное число исключено НАМЕРЕННО, и это не осторожность,
        а замер. Родительный падеж множественного числа у делового слова
        совпадает с именительным падежом мужской фамилии: «актов» ← «акт»,
        «балансов» ← «баланс», «банков» ← «банк». Перебор всех 1443 форм
        150 слов стоп-листа (07.08.2026) дал 70 таких совпадений — и ВСЕ 70
        оказались формами множественного числа, ни одной единственного.
        Без этого условия фамилия «Балансов» перестала бы маскироваться:
        словарь её как фамилию не знает, и страховка «нет словарного личного
        разбора» её НЕ спасает — проверено, страховки из одного признака мало.
        """
        for p in self._morph.parse(word):
            if p.normal_form not in stop_slova:
                continue
            if "plur" in p.tag.grammemes:
                continue
            return True
        return False
...
class Anonymizer:
    ...
    def _stop_slovo(self, value):
        """Значение — слово из стоп-листа: точной формой ИЛИ падежной.

        ⛔ Страховка от утечки: слово, которое словарь ЗНАЕТ как имя, фамилию
        или отчество, стоп-словом стать не может, даже если его лемма лежит
        в списке. Одной этой страховки МАЛО — замерено: фамилию «Балансов»
        словарь не знает вовсе, и спасает её только запрет на множественное
        число внутри `padezhnaya_forma_stop_slova` (замер там же).

        ⚠️ Условие «одиночное слово» — предусловие морфологии, а не страховка:
        pymorphy3 разбирает слово, а не фразу, и на многословном спане вернул бы
        мусор. Отдельным тестом не закрывается: снятие условия НЕ меняет ответ
        (проверено мутацией 07.08.2026), потому что лемма фразы в список всё
        равно не попадает. Называю прямо, чтобы условие не считали проверенным.
        """
        if value.lower() in NER_ROLE_STOPWORDS:
            return True
        if self.surnames is None or len(value.split()) != 1:
            return False
        if self.surnames.slovarnoe_lichnoe(value):
            return False
        return self.surnames.padezhnaya_forma_stop_slova(value, NER_ROLE_STOPWORDS)
    ...
    def _apply_ner(self, text):
        ...
        for start, stop, type_id, label, _ in reversed(self.ner.spans(_ner_view(text))):
            ...
            if re.search(r'\d', value) and not re.search(r'[а-яёa-z]', value):
                continue
            # ⛔ Здесь НЕ появится правила «заголовок капсом — не сущность».
            # Гипотеза «маскировать капс только при словарном личном разборе»
            # замерена 07.08.2026 и ОТВЕРГНУТА: на оси «фамилия против
            # заголовка» она безошибочна (34 слова), но на оси «название
            # компании» ломает маскировку у 5 из 5 проверенных названий
            # капсом — словарь не отличает «АЭРОФЛОТ» от «РЕЗЮМЕ» ничем.
            # Утечка дороже переусердствования, поэтому заголовки капсом
            # остаются лишней маской. Подробности — в HANDOFF.
            # Одиночное слово-роль из шапки документа — не ФИО
            if self._stop_slovo(value):
                continue
