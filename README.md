...
## Граница применимости: что инструмент делает и чего не делает

⚠️ Важное различие, которое легко упустить. Инструмент убирает **прямые
идентификаторы** — ФИО, ИНН, телефон, почту, адрес, счета. Именно они делают данные
персональными, и после замены документ можно обрабатывать, не передавая ПДн.

Но в тексте остаются **квази-идентификаторы** — город, профессия, должность,
обстоятельство, деталь биографии. По отдельности ни один из них персональными данными
не является; в связке они могут указывать на конкретного человека тому, кто этот круг
лиц уже знает.

**Замер 07.08.2026** на синтетическом корпусе из 10 заметок: читателю выдали короткие
досье на пять вымышленных персонажей и обезличенные тексты. По остаточным приметам
человек был определён правильно **в 10 случаях из 10**. Условия были заведомо
благоприятны для узнавания — досье почти дословно повторяли приметы из текста, — так
что это верхняя граница, а не типичный случай. Разбор — `retrospectives/2026-08-07
Замер — подъём абстракции против маскирования.md`.

Практический вывод:

| Сценарий | Инструмент помогает? |
| --- | --- |
| Документ уходит в облачную LLM на обработку | ✅ да — у модели нет досье на ваш круг лиц, а прямые идентификаторы удалены |
| Обезличенный текст показывают человеку, знающему участников | ⛔ нет — по остаточным приметам он узнает, о ком речь |
| Нужна публикация обезличенного кейса | ⚠️ недостаточно: приметы придётся обобщать вручную |

⛔ Поэтому корректная формулировка результата — «прямые идентификаторы удалены», а не
«человека невозможно узнать». Задача устойчивости к повторному опознанию решается
другими методами (обобщение, k-анонимность), и этот инструмент их не реализует.
...
Поэтому есть третий проход: слово с большой буквы маскируется как ФИО, если словарь
[pymorphy3](https://github.com/no-plagiarism/pymorphy3) считает его личным именем по
одному из двух признаков:

- **фамилия и не имя** (`Surn` без `Name`/`Patr`) — берёт «Литвинов»;
- **все разборы личные** — берёт имена и отчества: «Марина», «Ксения», «Сергеевич».

Ни один признак по отдельности не годится: первый пропускает имена (а именно имя стояло
первой строкой на реальной визитке и уходило наружу), второй теряет «Литвинов».

Условие «есть нарицательное чтение» защищает решение проекта не маскировать обычные
слова: «Вера в успех», «Роман с продолжением», «Любовь и голуби» остаются нетронутыми,
потому что у всех трёх слов есть значение обычного слова.
...
Кроме кнопок и импорта модуля, обезличиватель умеет работать службой — чтобы
потребители не носили копию ядра у себя и не отставали от исправлений.
Полное руководство: [docs/http-rezhim.md](docs/http-rezhim.md).

```bash
pip3 install -r requirements-api.txt
ANONYMIZER_TOKEN=$(python3 -c "import secrets;print(secrets.token_urlsafe(32))") \
  uvicorn api:app --host 127.0.0.1 --port 8081
```

| Метод | Эндпоинт | Что делает |
| --- | --- | --- |
| POST | `/anonymize` | `{"text", "use_ner", "use_dict"}` → `{"text", "keys", "stats"}` |
| POST | `/restore` | `{"text", "keys"}` → `{"text"}` |
| GET | `/health` | состояние слоёв, `ready`, версия, отпечаток ядра |

Переменные окружения: `ANONYMIZER_TOKEN` (обязательна — **без неё служба не
стартует**), `ANONYMIZER_MAX_BODY` (предел тела, по умолчанию 256 КиБ),
`ANONYMIZER_LOG_LEVEL`, `ANONYMIZER_DOCS`.

Свойства, ради которых режим сделан именно так:

- **Ключи возвращаются вызывающему и нигде не хранятся** — ни файла, ни кеша.
- **В журнал идут только счётчики** — ни текста, ни ключей, ни фрагментов
  значений; текст исключения наружу не отдаётся (движок кладёт в него фрагмент
  разбираемых данных).
- **Fail-closed**: неполный щит (`ready: false`) — это отказ `503`, а не тихая
  работа без слоя. `/health` в таком состоянии тоже отвечает `503`.
- Все запросы, кроме `/health`, требуют `Authorization: Bearer <token>`.
...
⚠️ Тесты HTTP-режима (`tests/test_api.py`) требуют `fastapi` и `httpx`. Без них
файл целиком уходит в `skip`, **а прогон рапортует `OK`** — увидев skip, считать
HTTP-режим непроверенным.
...
Сторонние библиотеки (`natasha`, `pymorphy3` и др.) сохраняют собственные лицензии.

Версии, опубликованные до 04.08.2026 под MIT, остаются доступны на прежних условиях —
выданная лицензия не отзывается.
